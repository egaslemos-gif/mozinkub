...
3. Em cada escrita admin: bloqueio Blob → refresca o snapshot mais recente → grava → reenvia. Evita que uma instância serverless “antiga” apague imagens de outro projecto.
4. **Durante `next build` o persist está desligado** — um deploy não sobrescreve a base de produção com o seed.
5. O seed é **idempotente**: não apaga galeria, slides, timeline, nem sobrescreve `logoUrl` / `coverUrl` já existentes.
