 ...
 * 1) Cloudinary — produção (CLOUDINARY_URL ou CLOUDINARY_*)
 * 2) Vercel Blob — só se Cloudinary não estiver configurado
...
> {
  ...
  if (isCloudinaryConfigured()) {
    console.info("[upload] using Cloudinary");
    return uploadToCloudinary(bytes, name, ext);
  }

  console.warn(
    "[upload] Cloudinary not configured — set CLOUDINARY_URL or CLOUDINARY_CLOUD_NAME/API_KEY/API_SECRET",
  );
      ...
      if (process.env.VERCEL) {
        return {
          ...
          error: blobUploadErrorMessage(
            err,
            "Cloudinary não configurado e Vercel Blob indisponível. " +
              "No Vercel, adicione CLOUDINARY_URL (ou as 3 variáveis) em Production e faça Redeploy.",
          ),
        };
      }
    }
  ...
  if (process.env.VERCEL) {
    return {
      ok: false,
      error:
        "Upload indisponível: configure CLOUDINARY_URL (ou CLOUDINARY_CLOUD_NAME + API_KEY + API_SECRET) no Vercel Production e faça Redeploy.",
    };
  }
